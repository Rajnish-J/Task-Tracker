...
// Server view shared by the personal and team section pages. Creating nested
// sections/projects is structure work — owner-only inside a team.
export async function SectionBoardView({
  sectionId,
  taskId,
  teamId,
}: {
  ...
  const [space, { section, lanes }, { tree }] = await Promise.all([
    getSpaceContext(teamId),
    getSectionBoard(sectionId, teamId),
    getSectionsTree(teamId),
  ]);

  const totalTasks = lanes.reduce((sum, lane) => sum + lane.tasks.length, 0);
  const sheet = taskId ? await getTaskForSheet(taskId, teamId) : null;
